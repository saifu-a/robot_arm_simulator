...
def jointStatePublisher():
    ...
    data_publisher = rospy.Publisher(
...
def generateNewTargets():
    ...
    global TARGET_JOINT_POSITIONS
    TARGET_JOINT_POSITIONS = [random.uniform(
        revolute_limits[0], revolute_limits[1]) for i in range(4)]
    TARGET_JOINT_POSITIONS.append(random.uniform(
        prismatic_limits[0], prismatic_limits[1]))

    if (len(JOINT_POSITIONS)) and (len(TARGET_JOINT_POSITIONS)):
        global POSITION_STEPS
        POSITION_STEPS = [(target-current)/float(NUM_INTERPOLATIONS)
                          for (target, current) in zip(TARGET_JOINT_POSITIONS, JOINT_POSITIONS)]

    print("Current Joint Positions: %s" % (str(JOINT_POSITIONS)))
    print("New Target joint positions: %s and steps: %s" %
          (str(TARGET_JOINT_POSITIONS), str(POSITION_STEPS)))
...
if __name__ == "__main__":
    try:
        ...
        data_subscriber = rospy.Subscriber(
